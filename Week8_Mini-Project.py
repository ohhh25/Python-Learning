...
debris_image = simplegui.load_image(
    "http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/"
        "debris2_blue.png")
...
soundtrack = simplegui.load_sound(
    "http://commondatastorage.googleapis.com/codeskulptor-assets/sounddogs/"
        "soundtrack.mp3")
...
# please do not redistribute without permission from Emiel at 
...
def process_sprite_group(sprite_group, canvas):
    iterable_set = set(sprite_group)
    rs = set([])
    for sprites in iterable_set:
        if sprites.update():
            rs.add(sprites)
        else:
            sprites.draw(canvas)
    sprite_group.difference_update(rs)

def group_collide(group, sprite):
    global my_ship
    iterable_copy = set(group)
    rs = set([])
    for sp in iterable_copy:
...
def group_group_collide(group1, group2):
    iterable_copy = set(group1)
    rs_sprites = set([])
    num_of_collisions = 0
    for many_sprite in iterable_copy:
        if group_collide(group2, many_sprite):
            num_of_collisions += 1
            rs_sprites.add(many_sprite)
    group1.difference_update(rs_sprites)
...
def level_update(game_level, rock_vel, rock_avel):
    if game_level == 0:
        return
    else:
        rock_vel[0] *= (game_level * 10)
        rock_vel[1] *= (game_level * 10)
        rock_avel *= (game_level * 10)

# timer handler that spawns a rock    
def rock_spawner():
    global rock_group, score
    if len(list(rock_group)) >= 12:
        return
    else:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * 0.6 - 0.3, random.random() * 0.6 - 0.3]
        rock_avel = random.random() * 0.2 - 0.1
        hit =  dist(rock_pos, my_ship.get_position()) <= asteroid_info.get_radius() + my_ship.get_radius()
        if hit is True:
            return
        else:
            level = score / 120
            level_update(level, rock_vel, rock_avel)
